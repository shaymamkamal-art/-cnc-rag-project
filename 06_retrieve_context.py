...
def _get_collection():
    ...
    return _collection
def retrieve_context(query: str, top_k: int = 6, max_distance: float = 0.55) -> list[dict]:
    """Return the top_k most relevant chunks, filtered by relevance distance."""
    ...
    for i in range(len(result["ids"][0])):
        ...
        if distance > max_distance:
            continue  # too far from the query -- don't force irrelevant context on the LLM
        hits.append({
